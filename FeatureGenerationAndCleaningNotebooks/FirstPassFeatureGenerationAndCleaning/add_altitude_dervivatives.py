...
#dbname = "endomondo"
dbname = "endomondo"
conn = psycopg2.connect("dbname=%s user=pmulrooney"%dbname)
# pypy
#conn = psycopg2cffi.connect("dbname=%s user=pmulrooney"%dbname)
...
for _table in series_tables:
    print _table
    # For cleanup / testing
    query = "ALTER TABLE {} DROP COLUMN IF EXISTS altitude_first;".format(_table)
    cur.execute(query)

    query = "ALTER TABLE {} ADD COLUMN altitude_first numeric(10,5);".format(_table)
    cur.execute(query)
    conn.commit()
...
for _table in sorted(series_tables):
    ...
    for _workout in workoutIds:
        ...
        query = "select alt_difference, time_difference, time from (select altitude - lag(altitude) over (order by time) as alt_difference, time - lag(time) over (order by time) as time_difference, time from {} where workoutid = {} order by time) as foo;".format(_table, _workout)
        cur.execute(query)
        ...
        for _j in cur.fetchall():
            ...
            query = "update {} set altitude_first = {} where workoutid = {} and time = {};".format(_table, _tmp, _workout, _j[2])
            cur.execute(query)
        conn.commit()
...
for _table in series_tables:
    # For cleanup / testing
    query = "ALTER TABLE {} DROP COLUMN IF EXISTS altitude_second;".format(_table)
    cur.execute(query)

    query = "ALTER TABLE {} ADD COLUMN altitude_second numeric(10,5);".format(_table)
    cur.execute(query)
    conn.commit()
...
for _table in sorted(series_tables):
    ...
    for _workout in workoutIds:
        ...
        query = "select alt_difference, time_difference, time from (select altitude_first - lag(altitude_first) over (order by time) as alt_difference, time - lag(time) over (order by time) as time_difference, time from {} where workoutid = {} order by time) as foo;".format(_table, _workout)
        cur.execute(query)
        ...
        for _j in cur.fetchall():
            ...
            query = "update {} set altitude_second = {} where workoutid = {} and time = {};".format(_table, _tmp, _workout, _j[2])
