...
for _table in sorted(series_tables):
    ...
    for _workout in workoutIds:
        ...
        query = "select alt_difference, time_difference, time, sqrt(power(lat_difference, 2) + power(long_difference, 2)) as geo_distance from (select latitude - lag(latitude) over (order by time) as lat_difference, longitude - lag(longitude) over (order by time) as long_difference, speed - lag(speed) over (order by time) as alt_difference, time - lag(time) over (order by time) as time_difference, time from {} where workoutid = {} order by time) as foo;".format(_table, _workout)
        cur.execute(query)
        
        for _j in cur.fetchall():
            if _j[0] == None and _j[3] == None:
                continue
            ...
            _speed = "NULL"
            if _j[0] != None:
              _speed = round( _j[0] / _time, 5)
          
    
            query = "update {} set speed_first = {} where workoutid = {} and time = {};".format(_table, _speed, _workout, _j[2])
            cur.execute(query)
        conn.commit()
