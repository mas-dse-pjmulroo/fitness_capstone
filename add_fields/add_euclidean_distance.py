...
for _table in workout_tables:
    print _table
    # For cleanup / testing
    query = "ALTER TABLE {} DROP COLUMN if exists geo_distance;".format(_table)
    cur.execute(query)

    query = "ALTER TABLE {} ADD COLUMN geo_distance numeric(15,10);".format(_table)
    cur.execute(query)
    conn.commit()
...
for _table in sorted(series_tables):
    ...
    query = "select workoutid, sum(elapsed_distance) from {} group by workoutid".format(_table)
    cur.execute(query)
