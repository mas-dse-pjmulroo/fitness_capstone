...
for _table in series_tables:
    print _table
    # For cleanup / testing
    query = "ALTER TABLE {} DROP COLUMN if exists elapsed_distance;".format(_table)
    cur.execute(query)

    query = "ALTER TABLE {} ADD COLUMN elapsed_distance numeric(15,10);".format(_table)
    cur.execute(query)
    conn.commit()
...
for _table in sorted(series_tables):
    ...
    query = "select workoutId, distance / (series_length-1)  from {}_by_workout where series_length > 1".format(_table)
    cur.execute(query)

    _tmp = cur.fetchall()

    workoutIds = [_i[0] for _i in _tmp]
    distances = [_i[1] for _i in _tmp]
    
    print "    ", len(workoutIds)
    print ""
    
    for _idx, _workout in enumerate(workoutIds):

        count += 1

        _distance = distances[_idx]
        ...
        query = "select time, (row_number() over (order by time) - 1) * {} as elapsed_distance from {} where workoutid = {} order by time".format(_distance, _table, _workout) 
        ...
        for _j in cur.fetchall():
            ...
            if _tmp > 99999:
                _tmp = 99999
            query = "update {} set elapsed_distance = {} where workoutid = {} and time = {};".format(_table, _tmp, _workout, _j[0])
