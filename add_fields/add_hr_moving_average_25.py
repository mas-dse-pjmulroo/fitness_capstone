...
for _table in series_tables:
    print _table
    # For cleanup / testing
    query = "ALTER TABLE {} DROP COLUMN if exists heart_rate_ma_25;".format(_table)
    cur.execute(query)

    query = "ALTER TABLE {} ADD COLUMN heart_rate_ma_25 numeric(8,5);".format(_table)
    cur.execute(query)
    conn.commit()
...
for _table in sorted(series_tables):
    ...
    for _workout in workoutIds:
        ...
        query = "select time, avg(heart_rate) over (order by time rows between 25 preceding and current row) as mavg from {} where workoutid = {} order by time;".format(_table, _workout)
        cur.execute(query)
        ...
        for _j in cur.fetchall():
            ...
            try: 
                query = "update {} set heart_rate_ma_25 = {} where workoutid = {} and time = {};".format(_table, _tmp, _workout, _j[0])
                cur.execute(query)
            except:
                print query
                exit()
