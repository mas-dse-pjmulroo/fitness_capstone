...
for _table in sorted(workout_tables):
    ...
    for _user in userIds:
        ...
        query = "select workoutid, r.start_time - (select r2.start_time from {} r2 where r2.userid = {} order by start_time limit 1) from {} r where userid = {} order by start_time".format(_table, _user, _table, _user)
        cur.execute(query)
